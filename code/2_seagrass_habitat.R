...
clean_seagrass <- function(seagrass_data){
  seagrass_layer <- seagrass_data %>%
    ...
    # group all features by the seagrass field to then have a single feature
...
seagrass_tpwd <- st_read(dsn = tpwd_seagrass_dir, layer = "TPWD_Seagrass_8_17_12") %>%
  # due to overlapping features, need to make invalid data valid
  sf::st_make_valid() %>%
  # now run through the clean_function to have single data observation
  clean_seagrass()
...
seagrass_tpwd_christmas_west_bays <- st_read(dsn = tpwd_seagrass_dir, layer = "TPWD_ChristmasBay_WestBay_Seagrass") %>%
  # due to overlapping features, need to make invalid data valid
  sf::st_make_valid() %>%
  # now run through the clean_function to have single data observation
  clean_seagrass()
...
seagrass_noaa_2012 <- st_read(dsn = tpwd_seagrass_dir, layer = "NOAA_Seagrass_8_17_12") %>%
  # due to overlapping features, need to make invalid data valid
  sf::st_make_valid() %>%
  # now run through the clean_function to have single data observation
  clean_seagrass()
...
seagrass_noaa <- st_read(dsn = noaa_seagrass_dir, layer = "Seagrasses") %>%
  # now run through the clean_function to have single data observation
  clean_seagrass() %>%
...
seagrass_tx_ncei <- st_read(dsn = gom_seagrass_dir, layer = "Seagrass_ALFLMSTX") %>%
  clean_seagrass()
